...
string CliCommandPrintCommands::execute(const MvcController *controller, const vector<string> &args)
{
	cout << endl << "Avaiable commands" << endl << "=================" << endl;
	for (size_t i = 0; i < this->state_->actions_.size(); i++)
	{
		cout << this->state_->actions_.Index(i) << " - " << this->state_->actions_.at[i].helpdesc << endl;
	}
